...
main() {
  ...
  # ============================================================
  # 🔧 FIX : Modifier le fichier CIBLE du lien symbolique
  # ============================================================
  if [ -f "$zsh_path" ]; then
    # Si c'est un lien symbolique, on modifie la cible
    if [ -L "$zsh_path" ]; then
      target_file=$(readlink -f "$zsh_path")
      if [ -f "$target_file" ]; then
        sed -i "s/^$var_name=.*/$var_name=\"$theme_to_set\"/" "$target_file"
        notify-send -i "$iDIR/ja.png" "OMZ theme" "applied to dotfiles/zsh/.zshrc"
      else
        notify-send -i "$iDIR/error.png" "E-R-R-O-R" "Target of symlink not found!"
        exit 1
      fi
    else
      # Sinon, on modifie directement le fichier
      sed -i "s/^$var_name=.*/$var_name=\"$theme_to_set\"/" "$zsh_path"
      notify-send -i "$iDIR/ja.png" "OMZ theme" "applied to $zsh_path"
    fi
    notify-send -i "$iDIR/ja.png" "OMZ theme" "restart your terminal"
  else
